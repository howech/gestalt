...
function RemapConfig( options ) {
    var self = this;    
    var options = _.extend({ mapper: function(x) { return x; } }, options);
    RemapConfig.super_.call(this,options);

    var mapper = this._options_.mapper;
    if( _.isFunction(mapper) ) {
	this._mapper_ = this._options_.mapper;	
    } else {
	// Build a function to dereference an object
	this._mapper_ = function(old) { return mapper[old]; };
    }
    ...
    org.on('loaded', function() {
        self.keys();
	self.emit('loaded');
    });

    org.on('invalid',function(err,source) {
	self.emit('invalid',err,source);
    });
