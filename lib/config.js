...
p._on_change_ = function(change) {
	...
	if( _.isString( l.pattern )) {
	    ...
	    match = l.pattern( change.name, change );
	}
...
p._default_listeners_ = function(prefix) {
	...
	delete:   function() {},
	notReady: function() {self.emit('notReady'); },
	ready:    function() { if( self.ready()) { self.emit('ready'); } }
    };
}
...
//    } 
	...
	// we are at the end of the path
        if( _.isObject( value ) && !(value instanceof Configuration) && this._options_.destructure_assignments ) {
